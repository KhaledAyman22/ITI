...
namespace D08_PII
{
    ...
    ///use BCL Delegate (Built in) Predicate
    //public delegate bool Predicate<T>(T i);
    ...
    internal class Program
    {
        ...
        #region V3.0 User Defined Delegate Class
        ...
        static List<T> FindMatchedCondition<T>(List<T> Lst, Predicate<T> conFunPtr)
        {
            List<T> result = new();
            for (int i = 0; i < Lst?.Count; i++)
            {
        ...
        static void Main(string[] args)
        {
            ...
            Predicate<int> fPtr = new Predicate<int>(ChkFunctions.ChkOdd);
            ...
            //fPtr = ChkFunctions.ChkGrt50;
            ///Anonymous Method
            fPtr = delegate (int i) { return i > 50; };
            Lst2 = FindMatchedCondition(Lst, fPtr);

            Lst2 = FindMatchedCondition<int>(Lst, delegate (int i) { return i % 5 == 0; });


            Action<int, string> fPtr01 = delegate (int X, String Str) { Console.WriteLine($"{X}{Str}"); };
            Func<int, int, int> fPtr02 = delegate (int X, int Y) { return X + Y; };
            ...
            ///Lambda Expression
            //fPtr =  (int i)=> { return i > 50; };
            ///=> : Called Fat Arrow , Reads as Goes To

            Predicate<int> fPtr03 = i => i > 70;


            Lst2 = FindMatchedCondition<int>(Lst, x => x %4 ==0 );
            ...
            Predicate<string> sfPtr = O1.ChkChar;

            foreach (string item in FindMatchedCondition<string>(NameList, S=> Regex.Match(S,"ai").Success))
                //FindMatchedCondition<string>(NameList, sfPtr))
            {
