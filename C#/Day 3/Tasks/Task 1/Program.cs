    ...
    internal partial class Program
    {
        ...
        static public Employee ReadEmployee()
        {
            ...
            while (!valid)
            {
                ...
                SecurityLevel securityLevel; //= (SecurityLevel)Enum.Parse(typeof(SecurityLevel), Console.ReadLine());
                
                if (!Enum.TryParse(Console.ReadLine(), out securityLevel)) Console.WriteLine("Invalid security level!");
                else
                {
                    valid = true;
                    employee.SetSecurityLevel(securityLevel);
                }
            }
            ...
            while (!valid)
            {
                ...
                if (salary == 0) Console.WriteLine("Invalid salary!");
                else
                {
                    valid = true;
            ...
            while (!valid)
            {
                ...
                if (parts.Length == 3)
                {
                    ...
                    if (int.TryParse(parts[0], out day) && int.TryParse(parts[1], out month) && int.TryParse(parts[2], out year))
                    {
                        ...
                        {
                            valid = true;
                            employee.SetHireDate(new HireDate(day, month, year));
                        }
                    }
