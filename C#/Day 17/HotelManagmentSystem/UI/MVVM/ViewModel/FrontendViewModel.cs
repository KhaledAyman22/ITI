    ...
    class FrontendViewModel : INotifyPropertyChanged
    {
        ...
        public void SetTab3Grid()
        {
            ...
            //ReservationsSrc = context.Reservations.ToList();
                ...
                ReservationsSrc = DapperConnection.Connection.Query<Reservation>("select * from reservations").ToList();
        }
        ...
        public void SetTab4ListBoxes()
        {
            ...
            OccupiedSrc = DapperConnection.Connection.Query<string>("select concat(id, ' | ', firstname, ' | ', lastname, ' | ', phonenumber) from reservations where checkin = 'true'").ToList();
            ReservedSrc = DapperConnection.Connection.Query<string>("select concat(id, ' | ', firstname, ' | ', lastname, ' | ', phonenumber) from reservations where checkin = 'false'").ToList();
        }
        ...
        public List<Reservation> EditingMode()
        {
            if (IsEdit == Visibility.Hidden)
            {
                ...
                var tmp = DapperConnection.Connection.Query<Reservation>("select * from reservations").ToList();
        ...
        public void SearchFor(string text)
        {
            ...
            var tmp = DapperConnection.Connection.Query<Reservation>("select * from reservations").ToList();
            SearchSrc = tmp.Where(p => p.GetType().GetProperties().Any(prop => regex.IsMatch(prop?.GetValue(p)?.ToString()?.ToLower() ?? "")))
                           .ToList();
        }
        ...
        public bool DeleteWithID(object obj)
        {
            ...
            {
                //using HotelxContext context = new();
                //var res = context.Reservations.Where(i => i.Id == reservation.Id).Single();
                //context.Reservations.Remove(res);

                DapperConnection.Connection.Execute("delete from reservations where id = @id", new {id = reservation.Id});


                //context.SaveChanges();

                //SetComboSrc(context.Reservations.ToList());
                SetComboSrc(DapperConnection.Connection.Query<Reservation>("select * from reservations").ToList());
        ...
        public bool UpdateCurrent()
        {
            ...
            var res = DapperConnection.Connection.QuerySingleOrDefault<Reservation>($"select * from reservations where Id = {Reservation.Id}");
        ...
        {
            var res = DapperConnection.Connection.Query<Reservation>("select * from reservations");

            State = res.Select(i => i.State).Distinct().ToList();
            ...
            RoomNumber = res.Select(i => i.RoomNumber).Distinct().ToList();
            Gender = res.Select(i => i.Gender).Distinct().ToList();
        }
