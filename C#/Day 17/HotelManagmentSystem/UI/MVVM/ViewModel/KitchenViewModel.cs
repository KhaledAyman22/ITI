...
namespace UI.MVVM.ViewModel
{
    public class KitchenViewModel : INotifyPropertyChanged
    {
        ...
        public KitchenViewModel()
        {
            ...
            var res = DapperConnection.Connection.Query<Reservation>("select * from reservations where CheckIn = 'true' and SupplyStatus = 'false'").ToList();
            //context.Reservations.Where(i => i.CheckIn == true && i.SupplyStatus == false).ToList()
            SetQueueSrc(res);
        }
        ...
        public bool UpdateCurrent()
        {
            ...
            using HotelContext context = new();
            var res = DapperConnection.Connection.QuerySingleOrDefault<Reservation>("select * from reservations where id = @id", new { id = Reservation.Id });
        ...
        {
            OverviewSrc = DapperConnection.Connection.Query<Reservation>("select * from reservations").ToList();
        }
    }
}
